...
DEFAULT_PROJECTS = [
    {
        ...
        "goal": "Dental offices spend countless manual hours reading dense, unstructured EOB and EOP documents. Identifying specific denied procedure codes, cross-referencing patient data, and manually writing customized appeal letters creates a massive administrative bottleneck.",
        "result": "I engineered a specialized AI pipeline that ingests raw documents, uses vision-language models to extract structured tabular data from messy PDFs, flags denials against a rules engine, and instructs an LLM to draft context-aware, medically relevant appeal letters.",
        "architecture": [
    ...
    },
    {
        ...
        "goal": "Traditional event booking funnels suffer from high drop-off rates on mobile due to tedious multi-step form fields.",
        "result": "Created a fluid voice-first booking interface that streams audio packets directly to a backend orchestrator, dynamically resolves calendar availability via function calling.",
        "architecture": [
    ...
    },
    {
        ...
        "goal": "Enterprise documents remain siloed and difficult to query accurately without hallucinations or stale data leakage.",
        "result": "Architected an end-to-end RAG ecosystem featuring recursive chunking, dense vector indexing with FAISS, and an interactive web playground for custom multi-tenant chatbots.",
        "architecture": [
    ...
    },
    {
        ...
        "goal": "Maintaining thousands of SEO articles across multiple WordPress sites requires continuous keyword monitoring.",
        "result": "Built a scheduled Django worker that queries Search Console and Keyword Planner, detects decaying pages, generates structured schema & enriched content.",
        "architecture": [
    ...
    },
    {
        ...
        "goal": "Brand managers struggle to stay on top of daily trending industry news and formulate timely, engaging posts.",
        "result": "Engineered an autonomous agent that monitors competitor sources, aggregates high-resonance topics, generates hook-driven social copy.",
        "architecture": [
    ...
    },
    {
        ...
        "goal": "Dispatchers and field operators handle high-pressure logistics with dozens of trucks and dynamic route changes.",
        "result": "Implemented an intelligent copilot that understands role permissions, queries live telemetry, and coordinates work orders via structured tool calls.",
        "architecture": [
    ...
    },
    {
        ...
        "goal": "Large Shopify stores with 50,000+ SKU variants encounter frequent 404 links and duplicate meta descriptions.",
        "result": "Engineered an enterprise SEO co-pilot that continuously crawls catalogs, prioritizes high-revenue pages, generates fixes via multi-model ensemble.",
        "architecture": [
